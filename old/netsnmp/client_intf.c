...
static PyObject *
snmptest_g(PyObject *self, PyObject *args)
{
  int version;
  char *community;
  char *peer;
  ...
  int verbose = py_netsnmp_verbose();*/
    ...
    if (!PyArg_ParseTuple(args, "iss", &version,
			&community, &peer))
      return NULL;
    ...
    if (!status == STAT_SUCCESS)
        snmp_sess_perror("snmptest_g", ss);
    else {
    ...
    /* 
    return Py_BuildValue("(iii)", &anOIDt, &anOID, status);
    */
    end:
    ...
    Py_ssize_t *len)
{
  *val = NULL;
  if (obj && attr_name && PyObject_HasAttrString(obj, attr_name)) {
    char *attr = PyObject_GetAttrString(obj, attr_name);
    if (attr) {
      int retval;
      retval = PyUnicode_FromKindAndData(attr, val, len);
      Py_DECREF(attr);
      return retval;
    }
  }
...
static PyMethodDef ClientMethods[] = {
  ...
  {"test", snmptest_g, METH_VARARGS,
   "test"},
  {NULL, NULL, 0, NULL}        /* Sentinel */
};

static struct PyModuleDef client_intf = {
    PyModuleDef_HEAD_INIT,
    "client_intf",       /* m_name */
    "The Net-SNMP Python Interface",  /* m_doc */
    -1,                  /* m_size */
    ClientMethods,       /* m_methods */
    NULL,                /* m_reload */
    NULL,                /* m_traverse */
    NULL,                /* m_clear */
    NULL,                /* m_free */
...
{
    return PyModule_Create(&client_intf);
}
